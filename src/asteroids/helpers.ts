...
import type { Iposition } from './game.types'
/**
 ...
 */
export const asteroidVertices = (count:number, rad:number):Iposition[] => {
  let p = [];
 ...
 */
export const rotatePoint = (p: Iposition, center: Iposition, angle:number): Iposition => {
  return {
 ...
 */
export const randomNumBetweenExcluding = (min:number, max:number, exMin:number, exMax:number) => {
...
export const findAngle = (pos1:Iposition, pos2:Iposition):number => {
  return Math.round(Math.atan2((pos2.y-pos1.y),(pos2.x-pos1.x)) * 180 / Math.PI)
}
